...
function 获取所有文件($目录, $前缀 = '') {
    ...
    foreach ($文件 as $文件名) {
        ...
        if (is_dir($路径)) {
            ...
            $结果[$相对路径] = base64_encode(file_get_contents($路径));
        }
    }
    return $结果;
}
...
HTML;
    ...
    $html .= '<li class="' . ($php_ok ? 'ok' : 'error') . '">PHP版本: ' . $php_version . ' (需要5.4+) ' . ($php_ok ? '✓ 通过' : '✗ 不通过') . '</li>';

    $write_ok = is_writable($install_dir);
    $html .= '<li class="' . ($write_ok ? 'ok' : 'error') . '">目录可写: ' . ($write_ok ? '✓ 通过' : '✗ 不通过，请设置目录权限') . '</li>';

    $pdo_ok = extension_loaded('pdo');
    $html .= '<li class="' . ($pdo_ok ? 'ok' : 'error') . '">PDO扩展: ' . ($pdo_ok ? '✓ 通过' : '✗ 不通过') . '</li>';

    $session_ok = extension_loaded('session');
    $html .= '<li class="' . ($session_ok ? 'ok' : 'error') . '">Session扩展: ' . ($session_ok ? '✓ 通过' : '✗ 不通过') . '</li>';

    $html .= '</ul>';

    if ($php_ok && $write_ok && $pdo_ok && $session_ok) {
        $html .= '<p style="text-align:center;margin-top:30px;"><a href="?step=2" class="btn">开始安装</a></p>';
    } else {
        $html .= '<p class="error" style="text-align:center;">环境检测不通过，请修复后重试</p>';
    }
    ...
    foreach ($文件列表 as $相对路径 => $base64内容) {
        $完整路径 = $install_dir . '/' . $相对路径;
        $内容 = base64_decode($base64内容);
        if (写入文件($完整路径, $内容)) {
            $成功数++;
        } else {
            $失败数++;
...
HTML;
    ...
    $html .= '<p>成功: <span class="ok">' . $成功数 . '</span> 个文件</p>';
    $html .= '<p>失败: <span class="error">' . $失败数 . '</span> 个文件</p>';

    if ($失败数 > 0) {
        $html .= '<h3>失败文件列表:</h3><div class="file-list">';
        foreach ($失败列表 as $文件) {
            $html .= '<div class="file-item error">' . $文件 . '</div>';
        }
        $html .= '</div>';
    }

    if ($失败数 == 0) {
        $html .= '<div class="ok" style="text-align:center;font-size:18px;margin:20px 0;">安装成功！</div>';
        $html .= '<p style="text-align:center;"><b>默认管理员账号:</b> admin / admin123</p>';
        $html .= '<p style="text-align:center;margin-top:30px;"><a href="index.php" class="btn">进入游戏</a> &nbsp; <a href="admin.html" class="btn">管理后台</a></p>';
        $html .= '<p style="text-align:center;margin-top:20px;font-size:12px;color:#888;">提示：为了安全，安装完成后请删除install.php文件</p>';
    }
...
$install内容 .= "\n";
foreach ($文件列表 as $路径 => $base64内容) {
    $install内容 .= "        '" . str_replace("'", "\\'", $路径) . "' => '" . $base64内容 . "',\n";
}

$install内容 .= "    ];\n}\n?>";

$install内容 = str_replace('TIME_PLACEHOLDER', date('Y-m-d H:i:s'), $install内容);
